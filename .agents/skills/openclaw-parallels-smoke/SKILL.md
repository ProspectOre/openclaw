...
- The aggregate npm-update wrapper defaults to `Ubuntu 26.04`, matching `parallels-linux-smoke.sh`. If that VM is unavailable, its resolver prefers the newest versioned Ubuntu guest with major version `>= 24` and a fresh poweroff snapshot. Before counting any Linux lane as release evidence, verify the selected VM name contains a parseable Ubuntu major version `>= 24`; reject explicit below-floor or unversioned names and report the environment unsupported. The aggregate wrapper exposes `--macos-vm`; use `--vm` only with the standalone Linux smoke entrypoint. Do not assume any other host-specific VM name.
