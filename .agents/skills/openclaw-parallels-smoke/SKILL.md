...
- The aggregate npm-update wrapper must resolve the Linux VM with the same Ubuntu fallback policy as `parallels-linux-smoke.sh` before both fresh and update lanes. Treat any Ubuntu guest with major version `>= 24` as acceptable when the exact default VM is missing, preferring the newest versioned Ubuntu guest with a fresh poweroff snapshot. Do not assume a specific host, VM name, or Ubuntu point release; discover the available guests at runtime.
...
- Use the newest versioned Ubuntu guest with a fresh poweroff snapshot; discover the available guest and its version at runtime rather than relying on a host-specific default.
