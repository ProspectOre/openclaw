...
- The aggregate npm-update wrapper defaults to `Ubuntu 26.04`, matching `parallels-linux-smoke.sh`. If that VM is unavailable, its resolver prefers the newest versioned Ubuntu guest with major version `>= 24` and a fresh poweroff snapshot. If no supported guest exists, the resolver may fall back to another parseable Ubuntu name (including below 24); treat that lane as unsupported and do not count it as release evidence. The aggregate wrapper exposes `--macos-vm`; use `--vm` only with the standalone Linux smoke entrypoint. Do not assume any other host-specific VM name.
...
- Use the default `Ubuntu 26.04` guest with a fresh poweroff snapshot. If it is missing, discover and use the newest versioned Ubuntu guest with major version `>= 24`; pass `--vm` to pin another guest explicitly. If the resolver reports a below-24 fallback, stop and report the environment unsupported instead of treating the smoke as release evidence.
