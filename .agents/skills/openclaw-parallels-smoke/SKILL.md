...
- The aggregate npm-update wrapper defaults to `Ubuntu 26.04`, matching `parallels-linux-smoke.sh`. If that VM is unavailable, its resolver accepts Ubuntu guests with major version `>= 24` and prefers the newest versioned guest with a fresh poweroff snapshot; pass `--vm` when an explicit guest is required. Do not assume any other host-specific VM name.
