...
- The aggregate npm-update wrapper defaults to `Ubuntu 26.04`, matching `parallels-linux-smoke.sh`. If that VM is unavailable, its resolver accepts Ubuntu guests with major version `>= 24` and prefers the newest versioned guest with a fresh poweroff snapshot. The aggregate wrapper exposes `--macos-vm`; use `--vm` only with the standalone Linux smoke entrypoint. Do not assume any other host-specific VM name.
...
- Use the default `Ubuntu 26.04` guest with a fresh poweroff snapshot. If it is missing, discover and use the newest versioned Ubuntu guest with major version `>= 24`; pass `--vm` to pin another guest explicitly.
