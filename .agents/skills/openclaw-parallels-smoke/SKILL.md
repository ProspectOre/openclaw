...
- The aggregate npm-update wrapper defaults to `Ubuntu 26.04`, matching `parallels-linux-smoke.sh`. If that VM is unavailable, its resolver prefers the newest versioned Ubuntu guest with major version `>= 24`, without checking snapshots. Snapshot resolution happens afterward; a missing snapshot fails the lane rather than trying another guest. Verify a fresh poweroff snapshot exists on the selected guest before starting. Before counting any Linux lane as release evidence, verify the selected VM name contains a parseable Ubuntu major version `>= 24`; reject explicit below-floor or unversioned names and report the environment unsupported. The aggregate wrapper exposes `--macos-vm`; use `--vm` only with the standalone Linux smoke entrypoint. Do not assume any other host-specific VM name.
...
- Use the default `Ubuntu 26.04` guest with a fresh poweroff snapshot. If it is missing, discover and use the newest versioned Ubuntu guest with major version `>= 24`; pass `--vm` to pin another guest explicitly. Verify the selected name before the run; reject below-24 and unversioned guests, including explicit `--vm` choices, instead of treating the smoke as release evidence.
